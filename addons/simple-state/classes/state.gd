...
func change_state_name(name: String, force := false) -> void:
	change_state_node(get_node_or_null(name), force)
...
func change_state_node(node: State, force := false) -> void:
	if (
			node == null 
	...
	active_substate_changed.emit(_active_substate, old)
...
func enter(set_target: Node, set_animation_player : AnimationPlayer, set_debug_mode : bool) -> void:
	assert(set_target != null, "target must be set.")
	assert(set_animation_player != null, "animation_player must be set.")
	for child in get_children():
		assert(child is State, "A State should not have any children that are not other States.")
		child.choose_new_substate_requested.connect(_on_choose_new_substate_requested)
	...
	change_state_node(_choose_substate())
	_after_enter()
...
func exit() -> void:
	...
	for child in get_children(): 
		child.choose_new_substate_requested.disconnect(_on_choose_new_substate_requested)
...
func _on_choose_new_substate_requested() -> void:
	change_state_node(choose_substate(), true)
